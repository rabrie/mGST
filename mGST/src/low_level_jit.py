...
def dK_dMdM(X, K, E, rho, J, y, d, r, rK):
    ...
    for n in range(m):
        ...
        dM = np.ascontiguousarray(
            np.zeros((n_povm, d, rK, r)).astype(np.complex128))
        for i in range(len(j)):
            k = j[i]
            C = contract(X, j[:i])
            R = contract(X, j[i+1:])@rho
            for o in range(n_povm):
                L = E[o].conj()@C
                D_ind = L@X[k]@R-y[o, n]
                dM_loc = K[k].conj()@np.kron(L.reshape(pdim, pdim).T,
                                             R.reshape(pdim, pdim).T)
                dM[o, k, :, :] += dM_loc
                dK[k] += D_ind*dM_loc
        for o in range(n_povm):
                ...
                if j[i1] == k1:
                    for i2 in range(len(j)):
                        if j[i2] == k2:
                            ...
                            C = contract(
                                X, j[min(i1, i2)+1:max(i1, i2)]).reshape(pdim, pdim, pdim, pdim)
                            ...
                            for o in range(n_povm):
                                ...
                                dconjdK_loc = np.zeros(
                                    (rK**2, r, r)).astype(np.complex128)
                                for rk1 in range(rK):
                                    for rk2 in range(rK):
                                        if i1 < i2:
                                            ddK_loc[rk1*rK+rk2] = np.kron(
                                                L.reshape(pdim, pdim)@K[k1, rk1].conj(), R.reshape(
                                                    pdim, pdim)@K[k2, rk2].T.conj())@np.ascontiguousarray(
                                                        C.transpose(1, 3, 0, 2)).reshape(r, r)

                                            ddK_loc[rk1*rK+rk2] = np.ascontiguousarray(
                                                ddK_loc[rk1*rK+rk2].reshape(
                                                    pdim, pdim, pdim, pdim).transpose(
                                                        0, 3, 2, 1)).reshape(r, r)

                                            dconjdK_loc[rk1*rK+rk2] = np.kron(
                                                L.reshape(pdim, pdim)@K[k1, rk1].conj(), R.reshape(
                                                    pdim, pdim).T@K[k2, rk2].T)@np.ascontiguousarray(
                                                        C.transpose(1, 2, 3, 0)).reshape(r, r)

                                            dconjdK_loc[rk1*rK+rk2] = np.ascontiguousarray(
                                                dconjdK_loc[rk1*rK+rk2].reshape(
                                                    pdim, pdim, pdim, pdim).transpose(
                                                        0, 2, 3, 1)).reshape(r, r)
                                            ...
                                            ddK_loc[rk1*rK+rk2] = np.kron(
                                                L.reshape(pdim, pdim)@K[k2, rk2].conj(), R.reshape(
                                                    pdim, pdim)@K[k1, rk1].T.conj())@np.ascontiguousarray(
                                                        C.transpose(1, 3, 0, 2)).reshape(r, r)
                                            ...
                                            dconjdK_loc[rk1*rK+rk2] = np.kron(
                                                L.reshape(pdim, pdim).T@K[k2, rk2], R.reshape(
                                                    pdim, pdim)@K[k1, rk1].T.conj())@np.ascontiguousarray(
                                                        C.transpose((0, 3, 2, 1))).reshape(r, r)
...
def ddA_derivs(X, A, B, J, y, r, pdim, n_povm):
    ...
    for n in prange(m):
        ...
        for o in range(n_povm):
            ...
            dA[o] += D_ind*dM
            dconjdA[o] += D_ind * \
                np.kron(np.eye(pdim).astype(np.complex128),
                        R.reshape(pdim, pdim).T)
...
def ddB_derivs(X, A, B, J, y, r, pdim):
    ...
    for n in prange(m):
        ...
        for o in range(n_povm):
            ...
            dB += D_ind*dM
            dconjdB += D_ind * \
                np.kron(L.reshape(pdim, pdim), np.eye(
                    pdim).astype(np.complex128))
