...
def randU(n):
    ...
    """    
    return expm(1j*randHerm(n)).astype(np.complex128)
...
def randKrausSet(d,r,rK):
    ...
    for i in range(d):
        K[i,:,:,:] += randU(pdim*rK)[:,:pdim].reshape(rK,pdim,pdim)
    return K
...
def random_gs(d,r,rK,n_povm):
    ...
    Generates a random gate set with random gates, initial state and POVM
...
def generate_fids(d,l,m_f):
    ...
    Generate random ofiducial sequencecs
