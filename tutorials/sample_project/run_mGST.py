...
#                 [reporting.number_to_str(df_g.values[i,0], [percentiles_g_high[i,0], percentiles_g_low[i,0]], precision = 4) for i in range(len(gate_labels))],
#             r"\shortstack{Diamond distance \\ $\frac{1}{2}||\mathcal{U}_i - \hat{\mathcal{G}}_i||_{\diamond}$}": 
#                 [reporting.number_to_str(df_g.values[i,1], [percentiles_g_high[i,1], percentiles_g_low[i,1]], precision = 4) for i in range(len(gate_labels))],
#         })
...
#         df_g_rotation = DataFrame(np.array([[reporting.number_to_str(ax_ang_estimate[i,j], 
...
#                 [reporting.number_to_str(reporting.unitarities(X_opt_pp)[i], [percentiles_u_high[i], percentiles_u_low[i]],  precision = 4) for i in range(len(gate_labels))],
#         })        


#     df_o_err = DataFrame({
#         "Final cost": reporting.number_to_str(df_o.values[0,0], [percentiles_o_high[0,0], percentiles_o_low[0,0]], precision = 4),
...
#             [[reporting.number_to_str(ax_ang_estimate[i,j], precision = 3) for i in range(len(gate_labels))] for j in range(r)]).T)
...
#         })
#     df_o_err = DataFrame({
#         "Final cost": reporting.number_to_str(df_o.values[0,0], precision = 4),
...
if bootstrap_gen in ['Y', 'y', 'Yes', 'yes', 'YES']:
    ...
    if rK == 1:
        ...
        # Table of axes parameters
        df_g_rotation = DataFrame(np.array(
            [[reporting.number_to_str(ax_ang_estimate[i, j],
                                      [ax_ang_percentiles_high[i, j], ax_ang_percentiles_low[i, j]], precision=3)
              for i in range(len(gate_labels))] for j in range(r)]).T)
        ...
        df_g_rotation.T.style.to_latex('mGST_results/bloch_rotation_axes_coeffs.tex', column_format='c|*{%i}{c}' % (d),
                                       ...
                                       caption='Normalized rotation axes coefficient. Errors \
