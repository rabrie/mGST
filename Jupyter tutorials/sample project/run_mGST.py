...
# ###################################### Compute and save parameters for unitary approx.
   
if rK == 1 and bootstrap_gen in ['Y', 'y', 'Yes', 'yes', 'YES']:
...
#         r"Rotation angle $/\pi$": [uncertainty.number_to_str(ax_ang_estimate[i,0], [ax_ang_percentiles_high[i,0], ax_ang_percentiles_low[i,0]], precision = 4) for i in range(len(gate_labels))],
#         "Axes tilt vs. target (in ${}^{\circ}$)": [uncertainty.number_to_str(ax_tilt_to_target[i], precision = 4) for i in range(len(gate_labels))],
#         "Axes estimation error (in ${}^{\circ}$)": [uncertainty.number_to_str(ax_tilt_ang_percentile[0,i], precision = 4) for i in range(len(gate_labels))]
#     })
#     df_g_rotation.rename(index=gate_labels, inplace = True)

#     df_g_rotation.style.to_latex('mGST_results/bloch_rotation.tex', column_format = '*{6}{c}', position_float = 'centering', 
...
# filename = mGST_results + '/spam_errs_pp.pdf'
# reporting.generate_spam_err_pdf(filename, E_opt_pp.real.reshape(n_povm,1,r), rho_opt_pp.real.reshape(1,r), 
#                                 E_target_pp.real.reshape(n_povm,1,r), rho_target_pp.real.reshape(1,r),  basis_labels = Pauli_labels, spam2_content = 'target')
